...
WHERE extra!=1 and sinn !=1 and paquete =0";
					...
					<div class="portlet light portlet-fit">
					  <div class="portlet-title">
					    <div class="caption">
					      ...
					      <span class="caption-subject font-dark sbold uppercase"><h3>AGREGAR PRODUCTOS A <?=$nombre->nombre?>  </h3><span>
						  <?}?>
...
$('#btnAgregarPro').on('click', function() {
	...
	$('#tblProductos #tblProductosBody tr').each(function(){
	  ...
	  if(id==0){
		$('#tblProductosBody').append('<tr data-id="'+cod+'" data-cantidad="'+cantidad+'"  > <td style=" text-align:center;">' + producto + '</td> <td style=" text-align:right;">' + cantidad+'</td>  <td style=" text-align:center;" > <button type="button" class="btn btn-sm btn-danger btn-xs" id="btnBorrar">x</button> </td> </tr>');
	  }else{
	  ...
	  $('#tblProductos #tblProductosBody tr').each(function(){
	    count++;
            ...
            var cantidad=$(this).attr("data-cantidad");
  			...
  			$.post('ac/agrega_producto.php', { cantidad, producto,ingrediente},function(data){
				
	 		 if(data==1){
			
		 	 window.open("?Modulo=Productos", "_self");
				
				  //window.open("?Modulo=Ingredientes&id=<?=$idp+1?>", "_self");
	  		}else{
		  			console.log(data);
		 			 alert(data);
	  }
