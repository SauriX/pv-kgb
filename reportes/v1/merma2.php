...
WHERE merma.activo='1' AND merma.fecha BETWEEN '$fecha1' AND '$fecha2'";
...
<table width="730">
	<tr>	
		...
		<b>Reportes de Merma del <?=fechaTexto($fecha1);?> al <?=fechaTexto($fecha2);?></b><br/>
...
<table width="730" bgcolor="#e6e6e6">
	<tr>	
		...
		Merma del d&iacute;a <?=fechaTexto($ft['fecha'])?> a las <?=$ft['hora']?> por: <?=$ft['usuario']?> <? $id_detalle=$ft['id_merma']; ?>
		</td>
	</tr>
</table>
...
<?  $query2=mysql_query("SELECT productos.nombre, merma_detalle.cantidad FROM merma_detalle 
						JOIN productos ON productos.id_producto = merma_detalle.id_producto 
						WHERE merma_detalle.id_merma='$id_detalle' AND merma_detalle.activo='1'",$conexion);
