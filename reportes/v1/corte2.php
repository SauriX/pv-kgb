...
$sql="SELECT cortes.*,cortes.fecha as dia, usuarios.usuario as user FROM cortes 
...
<table width="730">
	<tr>	
		...
		<b>Reporte de Cortes<br>Del d&iacute;a <?=fechaTexto($fecha1);?> al <?=fechaTexto($fecha2);?></b><br/>
...
<table width="730" bgcolor="#e6e6e6">
	<tr>
		<td width="350" align="left" valign="middle" height="22" class="lista_info2"><b>Corte a las <?=$hora?> - <?=fechaTexto($fecha)?></b></td>
...
<?
 ...
 $sql="SELECT cancelaciones_detalle.*,productos.nombre as prodnombre FROM cancelaciones_detalle 
	...
	WHERE (id_cancelacion BETWEEN $id_cancelacion AND $id_cancelacion2)";
	$qx = mysql_query($sql);
