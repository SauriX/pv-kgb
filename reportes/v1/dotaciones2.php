...
$fecha1=$_POST['calendario2'];
$fecha2=$_POST['calendario3'];
$sql="SELECT dotaciones.id_dotacion,dotaciones.fecha,dotaciones.hora,usuarios.usuario 
FROM dotaciones JOIN usuarios ON dotaciones.id_usuario=usuarios.id_usuario 
WHERE dotaciones.activo='1' AND dotaciones.fecha BETWEEN '$fecha1' AND '$fecha2'";
...
<table width="730">
	<tr>	
		...
		<b>Reportes de Dotaciones del <?=fechaTexto($fecha1);?> al <?=fechaTexto($fecha2);?></b><br/>
...
<table width="730" bgcolor="#e6e6e6">
	<tr>	
		...
		Dotaci&oacute;n del d&iacute;a <?=fechaTexto($ft['fecha'])?> a las <?=$ft['hora']?> por: <?=$ft['usuario']?> <? $id_detalle=$ft['id_dotacion']; ?>
		</td>
	</tr>
</table>
...
<?  $query2=mysql_query("SELECT productos.nombre, dotaciones_detalle.cantidad FROM dotaciones_detalle JOIN productos ON productos.id_producto = dotaciones_detalle.id_producto WHERE dotaciones_detalle.id_dotacion='$id_detalle' AND dotaciones_detalle.activo='1'",$conexion);
