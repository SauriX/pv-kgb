...
<? if($n1){	?>
	...
	while($ft = mysql_fetch_assoc($q)){
		...
		/* echo 'http://tacoloco.mx/formatos/factura.php?NomArch='.$ft['xml'].'&NombrePDF='.$ft['uuid'].'.pdf<br>'; */
	?>
			...
			switch($metodo){
					...
					break;
							...
							<td id="acciones_<?=$ft['id_factura']?>" style="text-align: right">
								...
								<div class="btn-group">
								  <button type="button" class="btn btn-primary btn-xs dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Opciones <span class="caret"></span>
								  </button>
								  <ul class="dropdown-menu pull-left">
								    <li><a href="http://tacoloco.mx/facturacion/pdf/<?=$ft['pdf']?>" data-pdf="http://tacoloco.mx/d.php?r=<?=$ft['uuid']?>&t=p" class="view-pdf">PDF</a></li>
								    <? if($est=="Vigente"){ ?>
								    <li><a href="http://tacoloco.mx/d.php?r=<?=$ft['uuid']?>&t=x">XML</a>
								    <li><a href="#" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#enviarEmail" data-id="<?=$ft['id_factura']?>" data-email="<?=$ft['email']?>">Email</a></li>
								    <? } ?>
								     <li><a href="#" onclick="ticket_factura(<?=$ft['id_factura']?>);" >Ticket Factura</a>
								    <? if($ft['estado']=='VIGENTE'){ ?>
...
?>
<center><h4>No hay facturas en esa fecha (<?=$fecha?>).</h4></center>
<?
}
?>
				</div>
...
<!--Formulario -->
	...
	function ticket_factura(id_factura){
		//alert(xml);
		$.post('includes/ticket_factura_Nuevo.php', { id_factura: ""+id_factura+"" },
			function(data){
				//alert(data);
			});
	}
