				...
				if($ft['paquete']==0 & $ft['extra']==0 & $ft['sinn']==0){
				  ...
				  <td align="right">
						...
						<div class="btn-group btn_<?=$proyecto->id_proyecto?>">
                                        ...
                                        <ul class="dropdown-menu">
											...
											<?if($s_tipo==1){?>
											   ...
											   <a href="javascript:;" data-toggle="modal" data-backdrop="static" data-keyboard="false" onclick="javascript:eliminar(<?=$ft['id_producto']?>)" >Eliminar</a>
											<?}?>
                                            </li>
                                        </ul>
                                    </div>
				    ...
				    <? }else{ ?>
						...
						<span class="label label-danger link btn_<?=$ft['id_producto']?>" onclick="javascript:eliminar(<?=$ft['id_producto']?>)">Eliminar</span
				    <? } ?>
				  </td>
				</tr>
			  <? } ?>
...
<!--Formulario -->
		<form id="frm" class="form-horizontal">
			...
			<div class="form-group">
				<label for="categoria" class="col-md-3 control-label">Color</label>
				<div class="col-md-7">
					<select class="form-control" id="id_color" name="color">
						<option selected="selected">Seleccione una</option>
				
							<option value="#CC3333">Rojo</option>
							<option value="#FFCC33">Amarillo</option>
							<option value="#339933">Verde</option>
							<option value="#333333">Negro</option>
							<option value="#3399FF">Azul</option>
			...
			<div class="form-group">
				<label for="precio_venta" class="col-md-3 control-label">Imagen</label>
				<div class="col-md-9">
					<input type="file" class="form-control " name="imagen"  autocomplete="off">
				</div>
			</div>
			<!--
			<div class="form-group">
...
<form id="categoria_nueva" class="form-horizontal oculto">
			...
			<div class="form-group">
				<label for="nombre" class="col-md-3 control-label">Nombre</label>
				<div class="col-md-9">
					<input type="text" class="form-control" name="nombre" id="nombre_categoria" autocomplete="off" maxlength="20">
				</div>
			</div>
			<div class="form-group">
...
function EditaProducto(){
	...
	$('.btn').hide();
	$('#load2').show();
	myForm = document.getElementById('frm_edita');
    formData = new FormData(myForm);
	var datos=$('#frm_edita').serialize();
/* 	fetch('ac/edita_producto.php', {
            method: 'POST',
            body: formData
...
function eliminar(id){
	$.post('ac/eliminar_producto.php', {  id: id },function(data){
		if(data==1){
			window.open("?Modulo=Productos", "_self");
		}else{
			$("#load_"+id+"").hide();
