...
$sql="SELECT productos_base.producto, dotaciones_detalle.* FROM dotaciones_detalle 
JOIN productos_base ON productos_base.id_base=dotaciones_detalle.id_producto
WHERE id_dotacion=$id_dotacion AND dotaciones_detalle.activo=0 ";
$q=mysql_query($sql);
$detalles=array();
...
$sq_corte="SELECT *  FROM dotaciones WHERE id_dotacion= $id_dotacion";
...
<table class="table table-striped table-hover " id="tabla" cellspacing="0" width="100%">
	...
	<tbody>
		...
		<tr id="tr_<?=$detalle->id_dotacion_detalle?>">
...
<script>
function borra(id){
	var valida = confirm("Al eliminar esta dotación se descontaran las existencias al producto.");
